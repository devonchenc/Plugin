...
BOOL CCurveWnd::GetHistogram()
{
	...
	m_nlenData = abs(m_pImage->GetPitch()) * m_nImageHeight;
	// 将图像源数据储存起来
	m_dataSrc = new BYTE[m_nlenData];
	m_curData = new BYTE[m_nlenData];
	// 获得图像数据首地址
	m_pBits = (BYTE*)m_pImage->GetBits();
	...
	for (int i = 0; i<m_nImageHeight; i++)
	{
		for (int j = 0; j<m_nImageWidth; j++)
		{
			...
			Pixel = m_dataSrc + i * m_pImage->GetPitch() + j * m_nChannel;
	...
	for (int i = 0, j; i<4; i++)
	{
		for (j = 0; j<256; j++)
		{
			m_pfHistogram[i][j] = m_pfHistogram[i][j] / m_nImageWidth / m_nImageHeight;
		}
	}
	return TRUE;
}
...
BOOL CCurveWnd::GrayTransform()
{
	...
	if (m_nChannelSelected == CHANNEL_RGB)
	{
		...
		for (int i = 0; i<m_nImageHeight; i++)
		{
			for (int j = 0; j<m_nImageWidth; j++)
			{
				Pixel = m_curData + i * m_pImage->GetPitch() + j * m_nChannel;
